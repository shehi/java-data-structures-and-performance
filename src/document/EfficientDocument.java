...
public class EfficientDocument extends Document {
    ...
    private void processText() {
        ...
        numWords = 0;
        numSyllables = 0;
        boolean aNewSentenceStarted = false;

        for (String token : tokens) {
            if (isWord(token)) {
                numWords++;
                numSyllables += countSyllables(token);
                aNewSentenceStarted = true;
            } else {
                numSentences++;
                aNewSentenceStarted = false;
            }
        }

        if (aNewSentenceStarted) {
            numSentences++;
        }
    }
    ...
    public static void main(String[] args) {
        testCase(new EfficientDocument("This is a test.  How many???  Senteeeeeeeeeences are here... there should be 5!  Right?"),
                16, 13, 5);
