...
public abstract class Document {
    ...
    protected int countSyllables(String word) {
        ...
        for (int k = 0; k < wordInLowercase.length(); k++) {
            char curChar = wordInLowercase.charAt(k);
            if (vowels.contains(curChar)) {
                if (curChar == 'e' && k == wordInLowercase.length() - 1 && numberOfSyllables != 0) {
                    continue;
                } else if (k > 0) {
                    char prevChar = wordInLowercase.charAt(k - 1);
    ...
    public double getFleschScore() {
        ...
        return 206.835 - 1.015 * ((double) numWords / (double) numSentences) - 84.6 * ((double) numSyllables / (double) numWords);
    }
}
